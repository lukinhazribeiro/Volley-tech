...
export async function listCategorias() {
  ...
  const rows = await db
    .select({
      ...
      totalAtletas: sql<number>`(select count(*) from ${atletas} where ${atletas.categoriaId} = ${categorias.id} and ${atletas.userId} = ${userId})`,
    })
    .from(categorias)
...
export async function createCategoria(formData: FormData) {
  ...
  await db.insert(categorias).values({ nome, descricao, userId })
...
export async function deleteCategoria(id: number) {
  ...
  const [{ count }] = await db
    ...
    .from(atletas)
    .where(and(eq(atletas.categoriaId, id), eq(atletas.userId, userId)))
  if (Number(count) > 0) throw new Error("Não é possível excluir: há atletas vinculados a esta categoria.")
  await db.delete(categorias).where(and(eq(categorias.id, id), eq(categorias.userId, userId)))
  revalidatePath("/gestao/categorias")
}
