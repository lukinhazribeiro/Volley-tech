  ...
  return (
    ...
    >
      ...
      <div className="relative h-40 w-full overflow-hidden">
        ...
        {/* index + status over image */}
        <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
          <span className="font-mono text-sm font-semibold tracking-widest text-[var(--hub-text)] drop-shadow">
            {String(index).padStart(2, "0")}
          </span>
          <span className="flex items-center gap-1.5 rounded-full border border-[var(--hub-border)] bg-[var(--hub-bg-deep)]/70 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-[var(--hub-text)] backdrop-blur">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 hub-pulse-glow" />
            {status}
          </span>
        </div>
        {/* icon overlapping bottom of image */}
        <div className="absolute -bottom-7 left-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--hub-accent)] to-[var(--hub-accent-strong,#c2410c)] text-[var(--hub-bg-deep)] shadow-lg shadow-[var(--hub-accent)]/30 transition-all duration-300 group-hover:scale-105 group-hover:shadow-[var(--hub-accent)]/50">
