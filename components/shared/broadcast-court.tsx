...
// Quadra de voleibol no estilo "transmissão" (perspectiva, duas equipes lado a
// lado com a rede ao centro). É o VISUAL PADRÃO compartilhado pelos três Scouts
// (Action, Volleibol e View). Não é uma área de coleta: serve para visualizar
// posições, placar de saque e elenco em quadra, tanto na coleta quanto para
// quem acompanha ao vivo em outro aparelho.
//
// O tipo de célula é genérico de propósito, para não acoplar a nenhum dos
// modelos internos de cada Scout — cada um monta suas células neste formato.

export type BroadcastSide = "A" | "B"

export interface BroadcastCourtCell {
  /** Posição na quadra: "P1".."P6". */
  posicao: string
  /** Jogador nessa posição (ou vazio). */
  player?: { id: string; number: number | string; name: string } | null
  /** Rótulo de função já traduzido (ex.: "Levantador", "Central"). */
  roleLabel?: string
  isLibero?: boolean
  isSetter?: boolean
}
...
interface BroadcastCourtProps {
  nameA: string
  nameB: string
  cellsA: BroadcastCourtCell[]
  cellsB: BroadcastCourtCell[]
  serving?: BroadcastSide | null
  /** Toque num jogador em quadra (ex.: substituição). Opcional. */
  onPlayerTap?: (side: BroadcastSide, playerId: string) => void
}
...
function CourtHalf({
  ...
  side: BroadcastSide
  cells: BroadcastCourtCell[]
  cols: readonly (readonly string[])[]
  onPlayerTap?: (side: BroadcastSide, playerId: string) => void
}) {
  const byPos = (pos: string): BroadcastCourtCell =>
    cells.find((c) => c.posicao === pos) ?? { posicao: pos, player: null }
            ...
            return (
              <div key={pos} className="flex justify-center">
                <PlayerToken
                  cell={cell}
                  side={side}
                  onTap={onPlayerTap && cell.player ? () => onPlayerTap(side, cell.player!.id) : undefined}
                />
              </div>
            )
          })}
...
export function BroadcastCourt({ nameA, nameB, cellsA, cellsB, serving, onPlayerTap }: BroadcastCourtProps) {
  return (
